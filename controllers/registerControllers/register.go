...
func CreateNormalUser(c *gin.Context) { //用户注册,默认权限组PermGroupID=1(GUEST)
	...
	// 激活令牌存于 Redis，需先签发再投递；签发失败同样视为发送失败
	if activationToken, issueErr := userService.IssueActivationToken(user.ID); issueErr != nil {
		activationMailSent = false
		msg = "注册成功，但激活邮件发送失败，请在“我的”页面重新发送"
		fmt.Println("签发激活令牌失败:", issueErr)
	} else if cooldown, sendErr := mailService.SendActivationEmail(user.Email, activationToken); sendErr != nil {
		activationMailSent = false
		msg = "注册成功，但激活邮件发送失败，请在“我的”页面重新发送"
