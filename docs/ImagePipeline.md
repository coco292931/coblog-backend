...
| P0-b-1 元数据剥离 | **6~10h** | **最高** | 需先定「原图是否允许字节改变」；方向 / ICC 要专门处理；批量处理还要等 P1-7 |
| P1-7 `images` 表 | 4~8h | 中 | AutoMigrate 标签对齐；要写目录回填任务 |
| P1-8 内容寻址 | 2~3h | 中 | 需兼容老随机文件名；要与 P0-a-1 协调上线 |
| P1-9 按内容去重 | 2h + 设计 | **中高** | 与「孤儿清理」互斥（#17），必须先做决策 |
...
> ⚠️ 前提：**文件名永不复用**。一旦要做「重新压缩老图」，必须先做 P1-8 的内容寻址，否则 immutable 会把旧内容钉住一年。
> 因此**建议把 immutable 与 P1-8 合并成一次上线**；中间态维持现状（CF 给的 `max-age=43200`）即可。

**2. 压缩规则修正** 🔲

- **alpha 感知**：`.png` 分支改为「有真实透明像素才 `png.Encode`，否则 `jpeg.Encode` 并返回 `".jpg"`」
...
- **JPEG 质量**：`compress_quality` 已存在（当前 80，越界回退 80），无需新增；只需在 `appConfigs_example.yaml` 的注释里写明「仅对 JPEG 输出生效」。

**3. `.webp` 二选一** 🔲

- **引入解码器**（成本比初稿低）：`golang.org/x/image` **已是间接依赖**（`go.mod` 里 v0.0.0-20191009），vendor 里没有 `webp` 包只是因为 vendor 只收被 import 的包 → 加一行 `_ "golang.org/x/image/webp"` + `go mod vendor` 即可，**不需要新引模块**。
  - ⚠️ 该版本解码器**不支持动图 webp**（含 ANIM 段的会报错），行为要明确（拒收 或 只取首帧）。
  - ⚠️ 若顺手升到新版 `x/image`，vendor 里的 bmp/tiff 也会跟着变，需要重跑一遍图片单测。
- 或把 `.webp` 从白名单移除并明确报错（最省事，但用户上传 webp 会直接失败）
...
**验收（P0-a）**
- [ ] `curl -I` 静态图片能看到 `Cache-Control: ... immutable`
- [ ] 无 alpha 的 PNG 压出 `_c.jpg`；带 alpha 的仍为 `_c.png`（单测覆盖）
- [ ] **单测钉住「resize 之后再判断 alpha 会误判」这个坑**
- [ ] 压缩收益不足 5% 的图不再产生 `_c.*`（单测覆盖）
- [ ] 超高大图被限制到 `compress_max_height` 以内
- [ ] `.webp` 行为明确（能压 或 拒收），不再是静默降级
- [ ] 带 EXIF Orientation=6/8 的照片，原图与 `_c.*` 朝向一致（单测覆盖）
- [ ] 超大像素的图片被明确拒绝，不会 OOM

### P0-b — 需先决策：原图剥离元数据 🔲

- JPEG：段级过滤（保留 SOF/DQT/DHT/SOS，丢弃 APPn/COM），**无损**，约 50 行
- PNG：块级过滤（保留 IHDR/PLTE/IDAT/IEND，丢弃 tEXt/iTXt/zTXt/eXIf/tIME），约 30 行
- ⚠️ **必须保留 APP2（ICC 色彩配置文件）**，否则广色域照片会褪色；只丢 APP1(EXIF/XMP) / APP13(IPTC) / COM
- ⚠️ **方向冲突**：剥掉 EXIF 会连 `Orientation` 一起丢 → 竖拍照片的原图会躺倒（现在靠浏览器自动旋转是正的）。二选一：
  1. **烘焙方向**：先按 Orientation 转正再剥元数据（`imaging.AutoOrientation(true)` + 重编码）→ 视觉正确，但属重编码、有轻微画质损失，与「原图无损」矛盾；
  2. **保留 Orientation 标签**：只重写一个只含 `Orientation` 的 APP1 段（约 +150 行），其余元数据丢弃 → 真正无损，但代码更复杂。
- 快捷替代：对原图做一次 `jpeg.Encode(q=95)` 重编码（5 行，但同样有画质损失，且对 PNG 无效）
- ⚠️ **只对新增上传生效**；历史图片要批量处理，必须等 P1-7 的 `images` 表落地
- ⚠️ 与「契约不变」冲突：原图字节会被改写，需要先明确回答 **「原图」是「字节不变」还是「视觉正确」**

**验收（P0-b）**
- [ ] 下载原图已不含 EXIF/GPS
- [ ] 原图的 ICC 配置保留（广色域照片颜色不变）
- [ ] 竖拍照片原图朝向正确（不论选哪种方案）
- [ ] 元数据剥离失败时按「保留原图字节」降级，不产生半损坏文件

### P1 — 结构性改造（1~3 天）

**7. 建 `images` 表（地基，先做这个）**
...
⚠️ 保留老随机文件名的读取兼容，且**建议与 P0-a-1 的 immutable 合并发布**。
...
2. **P0-b（1 天）**：元数据剥离。**前提是先回答「原图是否允许字节改变」**；它只对新增上传生效，历史图片要批量处理必须等 P1-7。
3. **P1-7 `images` 表（4~8h）**：P1-8/9/10/11 的共同地基，必须最先做；含目录回填任务。
4. **P1-8 内容寻址 → 再上 P0-a-1 的 `immutable`**（合并发布；中间态维持 CF 的 `max-age=43200`）。
5. **P1-9 去重**：动手前先定 #17（永不删 or 引用计数）。
6. **P1-10 格式协商**：⚠️ 受 `CGO_ENABLED=0` 限制，**建议推迟到 P2**。
7. **P1-11 多档尺寸**：等真实移动端流量数据。

**注意事项**

1. **后端先行**：前端依赖 `?thumb=1` 与 `X-Image-Variant`，老后端会忽略参数（展示会退回原图，更重但不坏）。
2. **契约不变**：数据库里存的始终是**原图 URL**，压缩/格式/尺寸都只是"服务端在同一个 URL 后面做决策" → 各条可独立上线，正文与前端无需跟着改。
3. **immutable 与重压的冲突**：上 P0-a-1 之后，若将来要重新编码老图，URL 必须变（P1-8），否则边缘/浏览器会长期返回旧内容。
...
7. **单测素材**：需要一张 `Orientation=6/8` 的 JPEG、一张带 alpha 的 PNG、一张大尺寸长截图（生成脚本或小体积样本，注意仓库体积）。
